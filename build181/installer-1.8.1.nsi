...
Section "Install"
  ...
  DetailPrint "מתקין את המאגר המקומי כשכבת ליבה מעל 1.7.6, בלי שינוי בקבצי ה-UI המקוריים של 1.6.3."
  ExecWait '"$TEMP\HaYad181Payload\asar-tool\node.exe" "$TEMP\HaYad181Payload\apply-1.8.1.cjs" "$TEMP\HaYad181Payload"' $0
  ${If} $0 != 0
    ...
    Abort
  ${EndIf}
  MessageBox MB_ICONINFORMATION "עדכון 1.8.1 הושלם. המאגר המקומי משולב כעת כשכבת Core Library."
  RMDir /r "$TEMP\HaYad181Payload"
SectionEnd
