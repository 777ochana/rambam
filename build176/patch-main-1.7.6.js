...
if (!mainPath || !packagePath) throw new Error('Usage: patch-main-1.7.6.js <main.js> <package.json>');
...
const helpers = String.raw`// LOCAL_CORPUS_176 — backend only; original renderer/CSS are untouched.
// IMPORTANT: the corpus is an external Electron resource, not a file inside app.asar.
function localCorpusDirectory() { return _nodePath.default.join(process.resourcesPath, 'local-library'); }
...
function runLocalSql(sql) {
  ...
  if (!_nodeFs.default.existsSync(exe) || !_nodeFs.default.existsSync(db)) {
    console.warn('Local corpus unavailable at external resources path:', localCorpusDirectory());
    return [];
  }
  try {
    const out = (0, _nodeChildProcess.execFileSync)(exe, ['-readonly', '-json', db, sql], { windowsHide: true, encoding: 'utf8', maxBuffer: 32 * 1024 * 1024, timeout: 8000 });
...
function localCorpusCatalog() {
  ...
  corpusCatalogCache = rows.map(row => ({
    ...
    blocks: [], segmentCount: Number(row.segments || 0),
    createdAt: '2026-09-03T00:00:00.000Z', _bookKey: row.title
  }));
  console.log('LOCAL_CORPUS_176 catalog loaded:', corpusCatalogCache.length, 'books from', localCorpusDbPath());
...
// Restore the original 1.6.3 visual scale if a previous experimental build left a Chromium zoom state behind.
_electron.app.on('browser-window-created', (_event, window) => {
...
console.log('Patched original 1.6.3 backend to 1.7.6; renderer/CSS untouched; external local corpus path fixed.');
